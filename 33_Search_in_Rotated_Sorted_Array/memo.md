...
- 二分探索をするというのが基本方針。境界がある場合は二分探索できないので、二分探索ができるようにリストを分割する。
  - 以下のようなイメージ
    - [4,5,6,7,0,1,2]を[4, 5, 6], [7], [0], [1, 2]に分割する。各分割リストに対して二分探索すればtargetがあるかどうかを判定できる
  - 分割の方法（および二分探索の方法）
    - リストの前半[left, middle]と後半[middle + 1, right]に分割 (middle = (left + right) // 2)
    - 分割した各リストは、境界があるかないか、の二パターン
    - 境界がない（つまりsortedされた分割リスト）場合は、そのリストに対して二分探索をする。境界がある場合は、もう一度分割する（これを繰り返す）

...
- メモリサイズに厳しい制約があるときは、HashTableを使いたくない。
- 上記の問題をもう少しアレンジして考えてみる。
...
- `nums2`を先頭から順に走査し、各要素が`nums1`に含まれているかを確認する。含まれていれば残す。また、`nums2`の重複要素はスキップする。
- 含まれているかどうかの判定には二分探索を用いる。

...
- 問題文によると、テストケース全体で解の個数は高々 150 個である。感覚的にはノード数は1000程度には収まりそう。その程度の探索で済むと考えると、リストのコピー（高々 20 要素）を含めても、およそ `20 × 1000 = 2 * 10^4` ステップ程度。
- Python の実行速度を `10^7` ステップ/秒とすると、実行時間は約2ms程度。
